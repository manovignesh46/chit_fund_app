...
export const chitFundAPI = {
  ...
  getById: (id: number | undefined) => {
    if (!id) {
      return Promise.reject(new Error('Invalid chit fund ID'));
    }
    return fetchAPI<any>(`/chit-funds/consolidated?action=detail&id=${id}`);
  },
...
export const loanAPI = {
  ...
  getById: (id: number | string[] | undefined) => {
    ...
    const numericId = Array.isArray(id) ? parseInt(id[0], 10) : id;

    return fetchAPI<any>(`/loans/consolidated?action=detail&id=${numericId}`);
  },
...
export const memberAPI = {
  ...
  getById: (id: number | undefined) => {
    ...
    return fetchAPI<any>(`/members/consolidated?action=detail&id=${id}`);
  },
