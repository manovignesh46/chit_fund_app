              ...
              <>
                ...
                <div className="mb-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-gray-600">Chit Funds with Pending</p>
                      <p className="text-2xl font-bold text-blue-600">{chitFunds.length}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Members with Pending</p>
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {chitFunds.map((chitFund) => (
                    <div key={chitFund.id} className="border-2 border-blue-200 rounded-lg p-4 bg-blue-50">
                      ...
                      <div className="mb-3 pb-3 border-b border-blue-300">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{chitFund.frequency} Contributions</p>
                          </div>
                          <div className="text-right">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div className="bg-white p-2 rounded">
                            ...
                            <p className="font-semibold text-gray-700">{formatCurrency(chitFund.totalAmount)}</p>
                          </div>
                          <div className="bg-white p-2 rounded">
                            <p className="text-xs text-gray-500">Installment Amount</p>
                            <p className="font-semibold text-gray-700">{formatCurrency(chitFund.installmentAmount)}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Pending Members */}
                      <div>
                        <p className="text-sm font-semibold text-gray-700 mb-2">Pending Members ({chitFund.pendingMembers.length})</p>
                        <div className="space-y-2">
                          {chitFund.pendingMembers.map((member, index) => (
                            <div key={index} className="bg-white border border-gray-200 rounded p-3 flex items-center justify-between hover:shadow-sm transition">
