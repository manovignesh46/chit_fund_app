        ...
        <div className="flex flex-row flex-wrap gap-1 sm:gap-2 w-auto items-center">
          ...
          >
            ...
            >
              <path
                fill="currentColor"
                d="M16.862 3.487a2.25 2.25 0 113.182 3.182l-9.193 9.193a2.25 2.25 0 01-.708.471l-3.25 1.3a.75.75 0 01-.97-.97l1.3-3.25a2.25 2.25 0 01.471-.708l9.193-9.193zM19.5 6.75L17.25 4.5"
              />
            </svg>
            <span className="hidden sm:inline-flex items-center">
              <svg
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <p
                  className={`text-xl font-semibold ${
                    loan.missedPayments > 0 ? "text-red-600" : "text-green-600"
                  }`}
                >
              ...
              {loan.repaymentType === "Monthly" && (
                <div>
                  ...
                  <p className="text-xl font-semibold">
                    {formatCurrency(loan.documentCharge || 0)}
                  </p>
                </div>
              )}
              <div>
                <h3
                  ...
                  onClick={() => {
                    const profitElement =
                      document.getElementById("loan-profit");
                    const profitExplanation = document.getElementById(
                      "loan-profit-explanation"
                    );
                    if (profitElement) {
                ...
                <div>
                  <p
                    id="loan-profit"
                    className="text-xl font-semibold text-green-600 hidden"
                  >
                    {loan.repaymentType === "Monthly"
                      ? (() => {
                          ...
                          const onlyHasInterestOnlyPayments =
                            ...
                            loan.repayments.every(
                                (r: any) => r.paymentType?.toUpperCase() === 'INTEREST_ONLY' || r.paymentType === 'interestOnly'
                            );
                          ...
                          if (onlyHasInterestOnlyPayments) {
                            ...
                            const profit =
                              (loan.interestRate || 0) *
                              interestOnlyPaymentsCount;
                            console.log(
                              "Detected interest-only payments only case - profit:",
                              profit,
                              "from",
                              interestOnlyPaymentsCount,
                              "payments"
                          ...
                          // Sum of all interest-bearing payments (Interest Only or Regular)
                          // We exclude Document Charges from the count as they are added separately
                          const interestBearingDuesPaid = loan.repayments
                            ? loan.repayments.filter((r: any) => {
                                const type = r.paymentType?.toUpperCase();
                                return type === 'REGULAR' || type === 'INTEREST_ONLY' || r.paymentType === 'interestOnly' || !type;
                              }).length
                            : 0;
                          ...
                          // Calculate profit: (Interest Amount × Number of Dues Paid) + Document Charge
                          const totalProfit = (loan.interestRate * interestBearingDuesPaid) + documentCharge;

                          console.log("Calculated total profit:", {
                            totalProfit,
                            formula: `(${loan.interestRate} × ${interestBearingDuesPaid}) + ${documentCharge} = ${totalProfit}`,
                          });
                      ...
                      : // For weekly loans, profit is Total amount paid - Principle amount
                        formatCurrency(
                          loan.installmentAmount * loan.duration - loan.amount
                        )}
                  </p>
                  <p
                    id="loan-profit-explanation"
                    className="text-xs text-gray-500 mt-1 hidden"
                  >
                    {loan.repaymentType === "Monthly" ? (
                      (() => {
                        // SPECIAL CASE: For loans with only interest-only payments
                        const onlyHasInterestOnlyPayments =
                          ...
                          loan.repayments.length > 0 &&
                          loan.repayments.every(
                            (r: any) => r.paymentType === "interestOnly"
                          );
                        ...
                        if (onlyHasInterestOnlyPayments) {
                          const interestOnlyPaymentsCount = loan.repayments
                            ? loan.repayments.length
                            : 0;
                          return `Interest from ${interestOnlyPaymentsCount} payment${
                            interestOnlyPaymentsCount !== 1 ? "s" : ""
                          }`;
                        }
                        ...
                        const hasDocumentCharge =
                          loan.documentCharge && loan.documentCharge > 0;
                        ...
                        if (hasRegularPayments) {
                          ...
                          explanation += `Interest from ${months} regular payment${
                            months !== 1 ? "s" : ""
                          }`;
                        }
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loadingSchedules ? (
                  ...
                  <tr>
                    ...
                    >
                      <p className="mb-4">No payment schedules to display.</p>
                      <p className="mb-4 text-sm">
                        Payment schedules show all past payments (including
                        overdue) and the next upcoming payment if it's due
                        within 3 days.
                      </p>
                    </td>
                  </tr>
